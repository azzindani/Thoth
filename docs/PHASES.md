...
| Keyless loop-14 (2026-09-24) | no fixable failures (epmc + metrotransit recovered upstream); ship: health+who-don (new `outbreaks`, 30 DONs, 19 placed), cyber+sslbl (new `sslbl`, 85 C2 certificates, 7-day window); countries+United States; JMA r8 path + code table queued (needs geometry); IFRC GO queued; unit 51, collectors 289/289 | done |
| Keyless loop-13 (2026-09-24) | fix pass after host reboot: epa-ie dropped (newest page past the upstream's 30 s gateway, API ignores filters) → radiation+bfs-odl (BfS gamma dose-rate network, 1,581 stations, current picture); bitstamp → host-blocked list (Indonesian DNS filter); ship: new `certs` collector → cyber (cert-fr avis+alertes, cert-eu, cccs, jpcert; 146 advisories); unit 51, collectors 282/282 | done |
| Keyless loop-12 (2026-09-24) | fix pass: openalex — paced single retry on sporadic 429 + restart guard (`succeededWithin`, restarts no longer re-spend the daily credit budget, a failed latest run is retried); epmc = upstream 503 outage (recorded); research tests +4 | done |
| Keyless loop-11 (2026-09-24) | fix: reddit (full-text search 422s → newest listing + local hazard-title match, NEVER_FROZEN), om-flood (per-gauge errors — one stall failed all 8); ship batch36: oceans+pegelonline, weather+hko-warn; batch36 tests 5/5, collectors 265/265 | done |
| Keyless expansion batch35/loop-10 (2026-09-24) | fires+qld-fire/wa-dfes/act-esa (`wildfires` now per-source loaders: JSON, two-call, XML), disasters+katwarn/biwapp/lhp-floods/de-police (`warnings` BBK provider table), batch35 tests 4/4, collectors 257/257; loop now every 30 min | done |
| Fix-or-drop pass #1 (2026-09-24) | monitor failing 26 → see OUTSTANDING: 7 recovered by `lib/net.ts` (2.5 s happy-eyeballs attempt), 18 fixed in code (energy-charts ×11, copernicus-ems, adsb.lol, ocha, coops-pressure, epa-ie, crossref, openalex/arxiv pacing), 2 dropped (goldapi, on511), 8 host-blocked → operator decision; unit 51, collectors 250 | done |
