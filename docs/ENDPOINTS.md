...
| agency incidents + warnings (NEW 2026-09-24 batch34/loop-9) | CAL FIRE active incidents (acres + % contained; ≥10k ac & <50% critical) + NSW RFS major incidents (AU warning levels, the feed's own origin Point) + Emergency Management Victoria fire incidents/warnings ("Safe" skipped) → `fires` (new `wildfires` collector); Environment Canada weather alerts (MSC GeoMet OGC API, risk colour → severity, one marker per alerted region) → `weather`; Environment Agency flood warnings (England, levels 1–3, area coords cached) + German MoWaS civil-protection alerts (warnung.bund.de, CAP severity, footprint centroid, Cancel skipped) → `disasters` (new `warnings` collector). All current-picture: pruned after each successful poll, NEVER_FROZEN, changed payload shapes fail loudly instead of emptying the layer. **Probed live from the production host 2026-09-24: 216 rows, all located** |
| agency incidents + warnings #2 (NEW 2026-09-24 batch35/loop-10) | Queensland Fire Department bushfire warnings + incidents (AU warning levels; publisher's own lat/lon), WA DFES bushfire incidents + bushfire warnings/smoke alerts (`/v1/incidents` + `/v1/warnings`, level from the warning subtype; non-fire incidents skipped), ACT ESA GeoRSS (fire-agency items only — the same feed carries every ambulance job; local times converted with the feed's AEST/AEDT) → `fires` (`wildfires`); warnung.bund.de KATWARN, BIWAPP, LHP floods and police warnings → `disasters` (`warnings`, one provider table with MoWaS, 30 s timeout for the host's stalls). Current-picture, pruned, NEVER_FROZEN. **Probed live 2026-09-24: 92 new rows, all located** (the four German providers had nothing in force) |
| gauges + HK warnings (NEW 2026-09-24 batch36/loop-11) | Pegelonline (WSV, DL-DE/Zero) German federal waterway gauges → `oceans` (`rivers`): 8 reference gauges every run (Köln/Kaub/Maxau, Dresden/Magdeburg, Frankfurt (Oder), Passau/Hofkirchen) + any gauge above mean high water (watch) or highest navigable water (critical), pruned when it drops back; Hong Kong Observatory warnings in force (`warnsum`: typhoon signals No. 8+/red-black rain/tsunami critical) → `weather` (`warnings`). **Probed live 2026-09-24: 8 gauges (all below mean low water — Kaub 7 cm), HKO quiet** |
| radiation live pulse (NEW 2026-09-24) | BfS ODL (German Federal Office for Radiation Protection, keyless WFS `odlinfo_odl_1h_latest`) → `radiation` (`bfs-odl`): latest 1-hour gamma dose rate of every operating station (≈1,580 of 1,676; defective/test sites and readings older than 6 h skipped), cosmic/terrestrial split in meta; ≥0.3 µSv/h watch, ≥1 critical; rewritten only when the newest hour moves, pruned as a current picture. Replaces EPA Ireland radmon (dropped: newest page past the upstream's 30 s gateway). Probed live 2026-09-24: 1,581 stations, max 0.238 µSv/h |
| national CERT advisories (NEW 2026-09-24) | CERT-FR avis + alertes (RSS), CERT-EU security advisories (RSS, CET/CEST stamps), Canadian Centre for Cyber Security alerts & advisories (Atom; AL serial = alert, AV = advisory), JPCERT/CC English security alerts (RDF) → `cyber` (new `certs`): one row per advisory keyed by its reference (CERTFR-2026-AVI-1175, 2026-013, …-av26-960, at260026), CVEs extracted; exploitation named in the text (EN/FR) → critical, alerts → watch, vendor advisories → info. Probed live 2026-09-24 |
| outbreaks + C2 certificates (NEW 2026-09-24) | WHO Disease Outbreak News (keyless OData, newest 30) → `health` (new `outbreaks`, source `who-don`): title split into disease + places (last dash or comma; formal names like "Kingdom of Saudi Arabia" fall back to the short one), placed at the first named country's capital, every country in entities; Ebola/Bundibugyo/Marburg/Nipah/plague/MERS/avian influenza/Lassa/CCHF/smallpox critical, other DONs watch. abuse.ch SSLBL certificate blacklist → `cyber` (new `sslbl`): C2 TLS certificate SHA-1s listed in the last 7 days with malware family, pruned as the week rolls. Shared country table gains the United States (Tor exits and UNHCR place US rows too). Probed live 2026-09-24: 30 DONs (19 placed; the rest are Global/Multi-country), 85 certificates (Vidar 20, OffLoader 18) |
| Red Cross emergencies (NEW 2026-09-24) | IFRC GO (keyless) → `disasters` (new `emergencies`, source `ifrc-go`): every emergency whose disaster started in the last 90 days (`/event/?disaster_start_date__gte=`), placed at the first affected country's capital; IFRC severity Red → critical, Orange → watch, Yellow → info, an active Emergency Appeal lifts Yellow to watch; active appeals (`/appeal/?status=0`, DREF + Emergency Appeal) joined by event id → CHF requested/funded, funded %; an appeals outage keeps the events and notes it. Current picture, pruned past the window. Probed live 2026-09-24: 71 emergencies, all placed, 4 watch. ReliefWeb v2 needs an approved appname → Free-key list |
| Japan warnings + volcano levels (NEW 2026-09-24) | JMA's 2026 (r8) warning feed → `weather` (`warnings`, source `jma-warn`): each forecast office's latest report per data type (VPWW55–61) merged per class10 sub-area; kinds decoded with the r8 table from JMA's own warning page (level 50 special, 40 danger — new in 2026, 30 warning, 20 advisory); one marker per sub-area at its polygon's centre (class10s.json, fetched once; island parts don't rename or move an area). JMA eruption warnings → `volcanoes` (`hans`, source `jma-volcano`): the volcano block of warning.json, English level names, summit from volcano_list.json. Both current pictures, pruned. Probed live 2026-09-24: 24 sub-areas (frost/fog/dry advisories, all placed, English names), 14 volcanoes (Sakurajima & Tokachidake Level 3, 8 at Level 2 or near-crater/sea-area, Iwatesan Level 1). The old `data/warning/*.json` froze on 2026-05-28. JMA river-flood forecasts (`flood/data/r8/flood_xml.json`) queued — empty today |
